...
def collate_fn(raw_batch):
    ...
    # image
    img = torch.stack(raw_batch[0], dim=0)  # [B,3,H,W]
    img_mask = torch.stack([m.to(torch.bool) for m in raw_batch[1]], dim=0)  # [B,H,W]
    img_data = NestedTensor(img, img_mask)

    # text
    word_id = torch.stack([x.long() for x in raw_batch[2]], dim=0)           # [B,L]
    word_mask = torch.stack([x.to(torch.bool) for x in raw_batch[3]], dim=0) # [B,L]
    text_data = NestedTensor(word_id, word_mask)

    # bbox target
    bbox = torch.stack([x.float() if torch.is_tensor(x) else torch.tensor(x, dtype=torch.float32)
                        for x in raw_batch[4]], dim=0)  # [B,4]

    phrase = list(raw_batch[5])  

    bbox_ori = torch.stack([x.float() if torch.is_tensor(x) else torch.tensor(x, dtype=torch.float32)
                            for x in raw_batch[6]], dim=0)  # [B,4]

    # obj_mask: [B,1,H,W]
    obj_mask_list = []
    for m in raw_batch[7]:
        if not torch.is_tensor(m):
            m = torch.tensor(m)
        if m.dim() == 2:
            m = m.unsqueeze(0)   # [1,H,W]
        obj_mask_list.append(m.to(torch.bool))
    obj_mask = torch.stack(obj_mask_list, dim=0)  # [B,1,H,W]

    # kp fields
    kp_root = torch.stack([x.float() for x in raw_batch[8]], dim=0)  # [B,2]
    kp_tip  = torch.stack([x.float() for x in raw_batch[9]], dim=0)  # [B,2]
    kp_valid = torch.stack([x.long() for x in raw_batch[10]], dim=0).view(-1)      # [B]
    is_positive = torch.stack([x.long() for x in raw_batch[11]], dim=0).view(-1)   # [B]

    return (
        img_data, phrase, bbox, obj_mask,          # <-- key change: phrase instead of text_data
        kp_root, kp_tip, kp_valid, is_positive,
        bbox_ori,                                  # optional debug
        text_data                                  # optional debug (put at the end so model won't see it)
    )
...
def collate_fn_mim(raw_batch):
    ...
    obj_mask = torch.tensor(np.array(raw_batch[8]))  
    phrase = raw_batch[6] 
