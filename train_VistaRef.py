...
import models.VistaRef_model as VistaRef_model
...
def get_args_parser():
    ...
    # ===================== [MODIFICATION BEGIN] =====================
    parser.add_argument('--lambda_kp', type=float, default=1.0,
                        help='weight for keypoint regression loss (SmoothL1)')
    parser.add_argument('--lambda_hand_cls', type=float, default=1.0,
                        help='weight for hand presence classification loss (BCEWithLogits)')
    # optional (if you plan to add geometric losses next)
    parser.add_argument('--lambda_dir', type=float, default=0.0,
                        help='weight for direction cosine loss')
    parser.add_argument('--lambda_ray', type=float, default=0.0,
                        help='weight for ray distance loss')
    parser.add_argument('--lambda_front', type=float, default=0.0,
                        help='weight for front hinge loss')
    parser.add_argument('--front_margin', type=float, default=0.05,
                        help='margin for front hinge constraint (normalized)')

    parser.add_argument("--debug_loss", action="store_true")

    # EMA normalize
    parser.add_argument('--loss_ema_momentum', type=float, default=0.99)
    parser.add_argument('--loss_ema_eps', type=float, default=1e-6)
    parser.add_argument('--loss_ema_warmup_iters', type=int, default=100)

    parser.add_argument('--loss_orig_weight', type=float, default=0.7)
    parser.add_argument('--loss_new_weight', type=float, default=0.3)

    parser.add_argument('--loss_hand_ratio', type=float, default=0.4)
    parser.add_argument('--loss_kp_ratio', type=float, default=0.2)
    parser.add_argument('--loss_ray_ratio', type=float, default=0.3)
    parser.add_argument('--loss_angle_ratio', type=float, default=0.1)
...
class EMALossNormalizer:
    """
    Maintain EMA stats for each raw loss and provide normalized loss.

    normalized_loss = raw_loss / (ema(raw_loss) + eps)
    """

    def __init__(self, momentum=0.99, eps=1e-6, warmup_iters=100):
    ...
    @torch.no_grad()
    def update(self, loss_dict_for_stats):
        """
        loss_dict_for_stats:
            dict of detached scalar tensors, typically reduced across GPUs for consistency
        """
        self.num_updates += 1

        for k, v in loss_dict_for_stats.items():
            if not torch.is_tensor(v):
                continue
            val = v.detach()

            if k not in self.stats:
                self.stats[k] = val.clone()
            else:
                self.stats[k] = self.momentum * self.stats[k] + (1.0 - self.momentum) * val

    def normalize(self, raw_loss_dict):
        """
        Use current EMA stats to normalize LOCAL raw losses (with graph).
        """
        norm_dict = {}

        for k, raw_v in raw_loss_dict.items():
            if k not in self.stats:
                # first few iterations: no reliable stat yet, use raw directly
                norm_dict[k.replace("_raw", "_norm")] = raw_v
            else:
                if self.num_updates <= self.warmup_iters:
                    # warmup stage: use raw directly or very lightly normalized
                    norm_dict[k.replace("_raw", "_norm")] = raw_v
                else:
                    denom = self.stats[k].to(raw_v.device, raw_v.dtype)
                    norm_dict[k.replace("_raw", "_norm")] = raw_v / (denom + self.eps)
...
def main(args):
    ...
    loss_normalizer = EMALossNormalizer(
        ...
        eps=args.loss_ema_eps,
        warmup_iters=args.loss_ema_warmup_iters,
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print("\n" + "=" * 80)
        print(f"Epoch [{epoch}/{args.epochs - 1}] started")
        print(f"Current lr before train_one_epoch: {optimizer.param_groups[0]['lr']:.8e}")
        print("=" * 80)
        
        train_stats = train_one_epoch(args, model, data_loader_train, optimizer, device, epoch,
                                      epoch * num_training_steps_per_epoch, args.clip_max_norm,loss_normalizer)
        lr_scheduler.step()
        val_stats = validate(args, model, data_loader_val, device)
        ...
        print("\n[Epoch Summary]")
        print(f"epoch:                 {epoch}")
        print(f"lr(after scheduler):   {optimizer.param_groups[0]['lr']:.8e}")
        print(f"best_accu_so_far:      {best_accu:.6f}")
        ...
        for k in important_train_keys:
            if k in train_stats:
                print(f"train_{k:<22}: {train_stats[k]}")
        ...
        for k in important_val_keys:
            if k in val_stats:
                print(f"validation_{k:<17}: {val_stats[k]}")
        print()
        ...
        if args.output_dir and utils.is_main_process():
            with open(os.path.join(args.output_dir, "log.txt"), "a") as f:
                ...
                f.write(json.dumps(to_jsonable(log_stats)) + "\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('VistaRef training script', parents=[get_args_parser()])
